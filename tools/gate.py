...
Design notes (Czech adaptation of the gate written for an earlier novel project):
  * `status: stub` files are exempt from word bands and prose lints - the gate
    ...
    word bands only (style-guide section 9).
  * Prose scoping: markdown blockquote lines are ARTEFACT spans (spisové
    hlavičky, spisy, diffy, oběžníky).  Narration = prose minus dialogue minus
    artefacts.  Hard blacklists run on everything; register lists run on
    narration; said-bookisms are tag-position aware (only a hit following a
    closing double quote, same sentence, counts as a tag).
  * Wildcards in section 8 entries: a '*' ATTACHED to a word fragment is a
    suffix wildcard (`nemoh*` matches nemohl/nemohla); a '*' between spaces
    bridges up to 60 characters inside one sentence (`mráz * po zádech`).
  ...
  * The glossary check tolerates Czech declension: a capitalised word counts
    as known if it shares a stem (all but at most 3 trailing characters,
    at least 4 shared) with a word from the bible vocabulary.
...
try:  # keep em-dashes and diacritics printable in a Windows console
...
POVS = ("Tereza",)          # main-line POV holders; interludes are pov: none

EXPECT_CHAPTERS = 15
EXPECT_INTERLUDES = 6

THRESHOLDS = {
    "chapter_hard": (2800, 4200),      # style-guide 3/9: hard band
    "chapter_target": (3200, 3800),    # target band -> warn outside
    "interlude": (550, 1100),          # fixace hard band
    "simile_per_300": 1.5,             # jako by | jako když | jako
    "italics_per_1k": 2.0,             # emphasis italics, artefacts excluded
    "glossary_min_uses": 3,            # recurring capitalised terms
}

CZ_UP = "A-ZÁČĎÉĚÍŇÓŘŠŤÚŮÝŽ"
CZ_LO = "a-záčďéěíňóřšťúůýž"

SIMILE_RE = re.compile(r"\bjako by\b|\bjako když\b|\bjako\b", re.I)
ITALIC_RE = re.compile(r"(?<![\*\w])\*([^*\n]+)\*(?!\*)")
CAPS_RE = re.compile(
    rf"\b[{CZ_UP}][{CZ_LO}]{{2,}}(?:[ -][{CZ_UP}][{CZ_LO}]{{2,}})*\b")
SPIS_RE = re.compile(r"\bF-\d{4}-\d{4}-[A-Z]\b")
...
class Repo:
    def __init__(self, root: Path) -> None:
        ...
        self.invented = self.bible / "invented-canon.md"
...
def parse_cards(text: str) -> tuple[list[Card], dict[str, str]]:
    ...
    for m in re.finditer(r"^## Part (I{1,3}) - .*?\*(.+?)\*", text, re.M):
    ...
    for block in blocks:
        ...
        if kind == "chapter":
            ...
            part_raw = _meta_value(meta, "part")
            part = part_raw.split()[0] if part_raw else current_part
            pov = "none"
        ...
        payoffs = LEDGER_ID_RE.findall(right.partition("**register**")[0])
...
def parse_timeline(text: str) -> dict[str, tuple[str, str]]:
    """chapter id -> (pov, ISO date); chapter rows only, interludes exempt."""
...
def parse_vocabulary(repo: Repo) -> tuple[set[str], set[str]]:
    ...
    vocab: set[str] = set()
    italics: set[str] = set()
    word_re = re.compile(rf"\b[{CZ_UP}][{CZ_LO}]{{2,}}\b")
    for path in (repo.glossary, repo.characters, repo.invented):
        if not path.exists():
            continue
        text = repo.read(path)
        vocab.update(w for w in word_re.findall(text))
        italics.update(m.group(1).strip().lower()
                       for m in ITALIC_RE.finditer(text))
    return vocab, italics


def parse_spis_codes(text: str) -> set[str]:
    """Spis codes registered in bible/glossary.md."""
    return {m.group(0) for m in SPIS_RE.finditer(text)}
...
def pattern_hits(text: str, entry: str) -> list[str]:
    """Word-boundary, case-insensitive.  A '*' attached to a word fragment is
    a suffix wildcard (nemoh* -> nemohl/nemohla); a '*' between spaces bridges
    up to 60 characters inside one sentence (mráz * po zádech)."""
    pieces = entry.split("*")
    rx_parts: list[str] = [re.escape(pieces[0])]
    pos = len(pieces[0])          # index of the '*' in the raw entry
    for piece in pieces[1:]:
        left = entry[pos - 1] if pos > 0 else ""
        if left and not left.isspace():
            rx_parts.append(r"\w*")                     # suffix wildcard
        else:
            rx_parts.append(r"\b[^.!?\n]{0,60}?\b")     # in-sentence gap
        rx_parts.append(re.escape(piece))
        pos += 1 + len(piece)
    rx = re.compile(r"\b" + "".join(rx_parts) + r"\b", re.I)
...
def check_front_matter(ch: Chapter, rep: Report) -> None:
    ...
    if not re.fullmatch(r"([1-9]|1[0-5])|I-[1-6]", cid):
        rep.fail("front-matter", ch.rel, f"chapter: {cid!r} not 1-15 / I-1..I-6")
    if str(ch.fm.get("part", "")) not in PARTS:
        rep.fail("front-matter", ch.rel, f"part: {ch.fm.get('part')!r} not I..III")
    if str(ch.fm.get("pov", "")) not in (*POVS, "none"):
        rep.fail("front-matter", ch.rel,
                 f"pov: {ch.fm.get('pov')!r} not {'|'.join(POVS)}|none")
    ...
    if not re.fullmatch(r"\d{4}(-\d{2}(-\d{2})?)?", date):
        ...
        rep.fail("front-matter", ch.rel,
                 ...
                 "(a bare year is legal only for pov: none)")
...
def check_lists(ch: Chapter, sc: Scopes, lists: dict[str, list[str]],
    ...
    if str(ch.fm.get("status", "")) == "stub":
        return
    for name in ("banned-phrases", "kalky"):          # hard: any hit fails
        hits: list[str] = []
        for entry in lists.get(name, []):
            hits += pattern_hits(sc.body, entry)
        if hits:
            rep.fail(name, ch.rel, ", ".join(sorted({f"'{h}'" for h in hits})))

    soft = [("kalky-watch", sc.body), ("filter-words", sc.narration),
            ("epithets", sc.narration), ("overused-adverbs", sc.narration)]
...
def check_thresholds(ch: Chapter, sc: Scopes, italics_ok: set[str],
    ...
    if str(ch.fm.get("status", "")) == "stub" or sc.words < 50:
        return
    sim = list(SIMILE_RE.finditer(sc.prose))
    ...
    # Emphasis italics only (style-guide 5.1/9): foreign words on first use and
    # publication titles are legal italics; artefact quotations and long spans
    # (> 6 words) are artefact material, not emphasis.
    emphasis = []
    for m in ITALIC_RE.finditer(sc.prose):
        span = m.group(1).strip()
        if span.lower() in italics_ok:
            continue
        if not re.search(r"[^\W\d_]", span):
            continue
        if len(span.split()) > 6:
            continue
...
def _known_word(word: str, vocab: set[str]) -> bool:
    """Declension-tolerant lookup: exact, or shared stem of >= 4 chars with at
    most 3 differing trailing characters on either side (Ústavu ~ Ústav)."""
    if word in vocab:
        return True
    for v in vocab:
        p = 0
        for a, b in zip(word, v):
            if a != b:
                break
            p += 1
        if p >= 4 and len(word) - p <= 3 and len(v) - p <= 3:
            return True
    return False
...
def check_glossary(ch: Chapter, sc: Scopes, vocab: set[str], rep: Report) -> None:
    ...
    for line in sc.prose.splitlines():
        for m in CAPS_RE.finditer(line):
            ...
            if sentence_initial:
                ...
                # anything after it still counts (e.g. "Ten Ústav retrospekce")
                words = term.split(" ")
                if len(words) < 2:
                    continue
                term = " ".join(words[1:])
    ...
    for term, n in sorted(counts.items()):
        if n < THRESHOLDS["glossary_min_uses"]:
            continue
        if all(_known_word(w, vocab) for w in re.split(r"[ -]", term)):
            continue
...
def check_spisy(ch: Chapter, sc: Scopes, allowed: set[str], rep: Report) -> None:
    if str(ch.fm.get("status", "")) == "stub":
        return
    bad = sorted({m.group(0) for m in SPIS_RE.finditer(sc.body)} - allowed)
    if bad:
        rep.fail("spisy", ch.rel,
                 "spis code(s) with no entry in bible/glossary.md: "
                 + ", ".join(bad))
...
def check_timeline(chapters: list[Chapter], tl: dict[str, tuple[str, str]],
                   rep: Report) -> None:
    """Chapters only; interludes carry a bare year and are exempt entirely."""
    where = "bible/timeline.md"
    dated: list[tuple[str, str, str]] = []   # (cid, pov, date)
    for ch in chapters:
        cid = card_id_for(ch)
        if not re.fullmatch(r"ch(0[1-9]|1[0-5])", cid):
            continue
        date = str(ch.fm.get("date_in_story", ""))
        row = tl.get(cid)
        if row is None:
    ...
    globalseq = [(c, d) for c, _, d in dated]
    for (c1, d1), (c2, d2) in zip(globalseq, globalseq[1:]):
        if d2 < d1:
            rep.fail("timeline", where,
                     f"book order not monotonic: {c1} {d1} -> {c2} {d2}")
...
def audit_cards(repo: Repo, rep: Report) -> None:
    ...
    spis_ok = parse_spis_codes(repo.read(repo.glossary))
    ...
    if len(chapters) != EXPECT_CHAPTERS:
        rep.fail("cards", where,
                 f"{len(chapters)} chapter cards, expected {EXPECT_CHAPTERS}")
    if len(interludes) != EXPECT_INTERLUDES:
        rep.fail("cards", where,
                 f"{len(interludes)} interlude cards, expected {EXPECT_INTERLUDES}")
    ...
    for c in cards:
        ...
        bad = sorted({m.group(0) for m in SPIS_RE.finditer(c.body)} - spis_ok)
        if bad:
            rep.fail("cards", where,
                     f"{c.cid}: spis code(s) not in glossary.md: " + ", ".join(bad))
    ...
    print(f"POV: Tereza {len(chapters)}/{len(chapters)} chapters "
          f"(+ {len(interludes)} interludes pov none)")
...
def run_gate(repo: Repo, rep: Report, only: list[Path] | None = None) -> None:
    ...
    print(f"bible: {len(cards)} cards, {len(rows)} ledger rows, "
          f"{len(tl)} dated chapters, {len(spis_ok)} known spis codes, "
...
# Book order: interludes interleave after fixed chapters (chapter-grid header).
INTERLUDE_AFTER = {"I-1": 2, "I-2": 5, "I-3": 7, "I-4": 10, "I-5": 12, "I-6": 15}


def assemble(repo: Repo, out: Path) -> int:
    # Optional per-root localisation: book/assemble.yaml (flat key: value).
    # Keys: title, lang, part_label, interlude_label, part_I..part_III.
    # Absent file -> defaults below, part titles from bible/chapter-grid.md.
    cfg: dict[str, str] = {}
    ...
    def sort_key(ch: Chapter):
        ...
        if cid.startswith("ch"):
            return (int(cid[2:]), 0)
        return (INTERLUDE_AFTER.get(cid, 99), 1)
    ...
    for ch in sorted(chapters, key=sort_key):
        ...
        elif title.lower().startswith(inter_label.lower()):
            lines += [f"## {title}", ""]
        else:
...
Nemohla si pomoct, musela ten spis otevřít znovu. Realizovala, že složka Šedý Protokol je najednou tenčí, a dávalo jí to smysl asi jako počasí.
Viděla razítka, cítila prach a doslova okamžitě zadržela dech: spis F-2031-9999-X ležel navrchu, opravdu a skutečně, jako by ho tam někdo položil včera, jako když se karta vrací do balíčku sama.

"Nech toho," vyhrkla blondýna od vedlejšího stolu.

"Ve finále to dopadne stejně," zasyčel starší muž od okna, a byl s tím v pořádku.

Cítila, jak jí mráz pomalu leze po zádech. Náhle se o Šedý Protokol nikdo nehlásil; najednou to *nedávalo* vůbec *žádný* *smysl*. A Šedý Protokol přitom držela v ruce, jako by papír vážil víc než kámen.
"""
...
Krátké tělo, hluboko pod jakýmkoli pásmem — přesně to má tahle atrapa
předvést. Nic víc tu není.
"""
...
def selftest(repo: Repo) -> int:
    ...
    for name in ("style-guide.md", "chapter-grid.md", "foreshadowing-ledger.md",
                 "timeline.md", "glossary.md", "characters.md",
                 "invented-canon.md"):
        src = repo.bible / name
        if src.exists():
            shutil.copy2(src, fixture / "bible" / name)
    (fixture / "manuscript" / "part-1" / "ch01-kamen.md").write_text(
        BAD_CH01, encoding="utf-8")
    (fixture / "manuscript" / "part-1" / "ch03-dest.md").write_text(
    ...
    expected = {
        "front-matter": "key order broken in ch03 (title before part)",
        "card-mirror": "ch03 title/date do not match the grid card",
        "word-band": "both fixtures far under the 2,800 floor",
        "banned-phrases": "'nemohla si pomoct', 'zadržela dech', 'mráz * po zádech'",
        "kalky": "'realizovala, že', 'dávalo jí to smysl', 'byl* s tím v pořádku'",
        "kalky-watch": "'ve finále', 'najednou to' etc. in context",
        "filter-words": "'viděla' / 'cítila' in narration",
        "said-bookisms": "'vyhrkla', 'zasyčel' in tag position",
        "epithets": "'blondýna', 'starší muž' in narration",
        "overused-adverbs": "'náhle', 'najednou', 'okamžitě', 'doslova' pile-up",
        "similes": "'jako by' / 'jako když' / 'jako' cluster",
        "italics": "emphasis italics over the 2/1k budget",
        "glossary": "Šedý Protokol (x3) not in glossary.md",
        "spisy": "F-2031-9999-X has no glossary entry",
        "ledger": "P-T9-9 has no row; P-T1-1 pays in ch11",
        "timeline": "ch03 dated 2031-09-10, timeline says 2031-09-22",
        "scaffold": "19 cards with no file",
    }
    ...
    for check, why in expected.items():
        ok = check in fired
        print(f"  [{'FIRED' if ok else ' -- '}] {check:<17} {why}")
        if not ok:
